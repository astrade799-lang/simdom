  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <select value={currentSkpdId} onChange={(e) => updateFilter("skpdId", e.target.value)} className={`${selectClass} max-w-[180px]`}>
        ...
        {(currentSearch || currentStatus || currentSkpdId) && (
          <button onClick={() => { setSearchInput(""); window.location.href = pathname }}
            className="rounded-lg border border-gray-300 px-3 py-2.5 text-sm text-gray-500 hover:bg-gray-50 transition-colors">
            Reset
          </button>
        )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="px-4 py-3 text-left font-semibold text-gray-600">Jenis Kegiatan</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {laporans.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center text-gray-400">Tidak ada laporan ditemukan</td>
                </tr>
              ) : (
                laporans.map((lap, i) => (
                  <tr key={lap.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <p className="font-medium text-gray-900">{lap.jenisKegiatan}</p>
                      <p className="text-xs text-gray-400 mt-0.5 line-clamp-1">{lap.deskripsi}</p>
                      {lap.instruksi && (
                        <p className="text-xs text-blue-600 mt-0.5">📋 {lap.instruksi}</p>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <p className="font-medium text-gray-700">{lap.webApp.nama}</p>
                      <span className="text-xs rounded-md bg-blue-50 px-1.5 py-0.5 font-semibold text-blue-700">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-1">
                        {/* Konfirmasi button - Kabid & Super Admin, hanya untuk PENDING */}
                        {canKonfirmasi && lap.status === "PENDING" && (
                          <button
                            onClick={() => { setSelectedLaporan(lap); setIsKonfirmasiOpen(true) }}
                            className="rounded-lg px-2 py-1 text-xs font-medium text-white bg-green-600 hover:bg-green-700 transition-colors"
                            title="Konfirmasi / Beri Instruksi"
                          >
                            Tindakan
                          </button>
                        )}
                        {/* Edit - Admin & Super Admin, hanya untuk PENDING */}
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
            <h3 className="text-base font-semibold text-gray-900">Hapus Laporan?</h3>
            <p className="mt-2 text-sm text-gray-500">Data laporan akan dihapus permanen.</p>
